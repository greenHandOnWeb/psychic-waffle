...
comment on column public.images.gallery_category is 'single=用户上传的编辑底稿；collage=保存布局时生成的拼图成品图';
comment on column public.images.source_image_id is '拼图成品对应的编辑源单图（images.id），仅 collage 时有值';
