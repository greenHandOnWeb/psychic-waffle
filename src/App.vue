<template>
  <div class="flex min-h-full flex-col">
    <a
      href="#main-content"
      class="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[100] focus:rounded-lg focus:bg-sky-600 focus:px-4 focus:py-2 focus:text-sm focus:text-white focus:outline-none focus:ring-2 focus:ring-sky-400"
    >
      跳到主内容
    </a>
    <header
    ...
    >
      <div class="mx-auto flex max-w-6xl items-center justify-between gap-4">
        <RouterLink to="/" class="text-lg font-semibold text-white hover:text-sky-400">
          灵动画册
        </RouterLink>
    ...
    <main
      id="main-content"
      tabindex="-1"
      class="mx-auto w-full max-w-6xl flex-1 px-4 py-6 outline-none"
    >
      <RouterView />
    </main>
    <AppToaster />
    ...
    <TransitionRoot appear :show="aboutOpen" as="div" class="contents">
      <Dialog class="relative z-50" @close="aboutOpen = false">
        <TransitionChild
          as="div"
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <div class="w-full max-w-md">
              ...
              >
                <DialogTitle class="text-lg font-semibold text-white"> 关于灵动画册 </DialogTitle>
                <p class="mt-3 text-sm leading-relaxed text-slate-300">
                  上传图片、Mock AI 打标签，使用云端模板或自由排版生成拼图海报。默认开发环境为 Mock
                  数据；接入 Supabase 后请关闭 Mock 并执行迁移 SQL。
                </p>
                <button
                  type="button"
...
import { ensureMockSession, ensureSupabaseAuthSession, isSupabaseMockMode } from '@/lib/supabase';
