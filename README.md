...
- **拼团记录与音乐**：保存并生成拼团时，新拼团条目的 `layout` 与当前画布一致（含 `audioSegments`），从画廊打开该拼团再进编辑器可继续试听/叠加，不再丢失音乐分段。
...
4. **在控制台执行迁移 SQL**（见下一小节「如何在 SQL Editor 里跑迁移」）：按顺序跑完 `001` → `002` → `003`，`001` 里会创建 Storage 桶 `gallery-images`。
...
1. 在本机用编辑器打开项目里的三个文件（路径如下），**全选复制** 文件中的全部 SQL：
...
6. 执行完后到左侧 **Storage** 确认是否存在桶 **`gallery-images`**；到 **Table Editor** 确认是否有表 **`profiles`、`images`、`templates`、`likes`** 等。

**顺序不能乱**：必须先 `001`（库表 + 桶 + 基础 RLS），再 `002`（画廊分类字段），再 `003`（点赞与 RPC）。
